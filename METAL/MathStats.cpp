...
double gammp_METAL ( double a, double x )
   {
...
void gser_METAL ( double * gamser, double a, double x, double * gln)
   {
   int   n;
   double sum, del, ap;
...
void gcf_METAL ( double * gammcf, double a, double x, double * gln)
   {
   int i;
...
double gammp( double a, double x)
{
   ...
   double gln;
   if ( x < 0.0 || a <= 0.0) error("invalid args in fammp");
   if ( x < (a+1.0) )
   {
   /* use the series rep */
      double gamser = 0.0;
...
void gser( double *gamser, double a, double x, double *gln)
{
   ...
   *gln = gammln(a);
   if ( x <= 0.0)
   {
      if ( x < 0.0) error("x < 0 in rtn gser");
         *gamser = 0.0;
         return;
   }
   else
