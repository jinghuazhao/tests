...
QTL_labels <- filter(QTLs,!is.na(druggability_category)) %>%
              mutate(druggability_category=cols[druggability_category+1])
QTL_labels
...
circos.track(ylim = c(0, 1),
             panel.fun = function(x, y) {
               ...
               xlim = CELL_META$xlim
               ylim = CELL_META$ylim
               circos.rect(xlim[1], 0, xlim[2], 1, col = "white", cex = 0.3, lwd = 0.5 )
...
circos.track(ylim=c(0,1), track.height=0.05, bg.border=NA, panel.fun=function(x, y) {
             ...
             xlim=CELL_META$xlim
             ylim=CELL_META$ylim
             circos.genomicAxis(h="top", direction="inside", labels.cex=0.35, major.at=seq(0,1e10,5e7))})
