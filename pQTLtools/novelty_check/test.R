...
for(i in unique(dplyr::pull(METAL,Chromosome)))
{
   ...
   m <- dplyr::filter(METAL,Chromosome %in% i) %>%
        dplyr::select(chr,pos,uniprot,rsid,prot)
   bfile <- file.path(INF,"INTERVAL","per_chr",paste0("interval.imputed.olink.chr_",i))
   b2[[i]] <- novelty_check(u,m,ldops=list(bfile=bfile,plink=plink))
}
replication2 <- dplyr::filter(bind_rows(b2), r2>=0.8)
